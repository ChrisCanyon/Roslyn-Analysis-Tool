...
namespace DependencyAnalyzer.Parsers
{
    public class ManualResolutionParser(Solution solution, SolutionAnalyzer solutionAnalyzer) : BaseParser
    {
                ...
                if (ImplementsInterface(rootInvocation.RootClass, "Castle.MicroKernel.Registration.IWindsorInstaller"))
                {
                    ...
                    foreach (var project in projects)
                    {
                        ret.Add(new ManualDisposeInfo
                        {
                            ...
                            CodeSnippet = disposeInvocation.ToFullString().Trim(),
                            Project = project,
        ...
        private static string BuildSimpleInvocationPath(
            ...
            Document document)
        {
            ...
            return $"{document.Project.Name}:{Path.GetFileName(file)}:{line} - {invocation.ToFullString()}";
        }
        ...
        private static List<ManualResolveInfo> GetResolutionInfoFromInvocations(InvocationExpressionSyntax invocation, SemanticModel model, string project, string invocationPath)
        {
            ...
            if (containingType == null)
            {
                Console.WriteLine($"Could not find containing class for {invocation.ToFullString().Trim()}");
            }
